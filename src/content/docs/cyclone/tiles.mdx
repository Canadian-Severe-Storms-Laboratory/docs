...
<ParamField path="model" type="string" required>
  Model ID from [`/models`](/cyclone/models#models).
</ParamField>

<ParamField path="product" type="string" required>
  Product name from [`/products`](/cyclone/models#products). Case-sensitive.
</ParamField>
...
<ParamField query="run" type="string">
  Run stamp from [`/runs`](/cyclone/models#runs), such as `20260924T120000Z`. Required unless you pass `time`.
</ParamField>

<ParamField query="hour" type="integer" default="0">
  Index into the model's `forecast_hour_axis`. This is not the forecast hour itself; see [Runs and hours](/cyclone/overview#runs-and-hours).
</ParamField>
