...
<ParamField query="run" type="string" required>
  Run stamp from [`/runs`](/docs/cyclone/models#runs).
</ParamField>
...
<ParamField query="row, col" type="integer" required>
  Grid cell, as zero-based indices into the native grid. [`/models/coverage`](/docs/cyclone/models#coverage) gives the grid size as `ny` × `nx`.
</ParamField>
