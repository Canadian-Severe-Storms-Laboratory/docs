...
Raindrop stores point data: station observations, alerts, outlooks and reports. It runs as two processes that share one database. One ingests data; the other serves the [REST API](/raindrop/overview).
...
- **Ingest:** subscribes to MSC Datamart over AMQP for station reports and alerts as they're published, and polls other networks on fixed schedules. Readings are converted to [standard units](/raindrop/observations#units) and [quality-checked](/architecture/data-sources#quality-control) before they're stored.
...
- **Serve** is the [HTTP API](/cyclone/overview). It reprojects data to Web Mercator tiles on request.
