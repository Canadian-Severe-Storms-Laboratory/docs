...
Every value Raindrop stores belongs to a **series**: one variable at one station, measured one way. The `types` array in [station details](/raindrop/stations#station-details) lists a station's series. Each one has:
...
Returns the readings of one variable at one station between `start` and `end`. The window is subject to your plan's [lookback](/raindrop/overview#time-windows).
...
Readings from ECCC's feed arrive with ECCC's own verdict. Raindrop runs its own checks on any reading that isn't already marked `Good`, which covers every other network. Derived values take the worst quality of their inputs. For details, see [Quality control](/architecture/data-sources#quality-control).
